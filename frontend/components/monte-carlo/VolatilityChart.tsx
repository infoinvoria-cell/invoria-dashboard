...
export default function VolatilityChart({ theme, volatilitySeries }: Props) {
    ...
    <section className="glass-panel rounded-[24px] border p-4 min-[769px]:p-5" style={{ background: palette.panelBackground, borderColor: palette.border, boxShadow: `0 18px 44px rgba(0,0,0,0.30), 0 0 28px ${palette.glow}` }}>
      ...
      <div className="h-[290px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={volatilitySeries} margin={{ top: 10, right: 10, bottom: 6, left: 0 }}>
            ...
            />
            <Legend />
            <Line type="monotone" dataKey="realized" stroke={palette.accentSoft} strokeWidth={1.8} dot={false} name="Realized" />
            <Line type="monotone" dataKey="garch" stroke={palette.accent} strokeWidth={2.3} dot={false} name="GARCH" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </section>
