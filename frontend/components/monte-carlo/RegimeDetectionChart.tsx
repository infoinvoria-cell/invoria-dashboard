...
export default function RegimeDetectionChart({ theme, regimeSeries }: Props) {
    ...
    <section className="glass-panel rounded-[24px] border p-4 min-[769px]:p-5" style={{ background: palette.panelBackground, borderColor: palette.border, boxShadow: `0 18px 44px rgba(0,0,0,0.30), 0 0 28px ${palette.glow}` }}>
      ...
      <div className="h-[290px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={regimeSeries} margin={{ top: 10, right: 10, bottom: 6, left: 0 }}>
            ...
            />
            <Area type="monotone" dataKey="bull" stackId="1" stroke={palette.positive} fill={palette.positive} fillOpacity={0.45} />
            <Area type="monotone" dataKey="neutral" stackId="1" stroke={palette.accent} fill={palette.accent} fillOpacity={0.35} />
            <Area type="monotone" dataKey="bear" stackId="1" stroke={palette.negative} fill={palette.negative} fillOpacity={0.38} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </section>
